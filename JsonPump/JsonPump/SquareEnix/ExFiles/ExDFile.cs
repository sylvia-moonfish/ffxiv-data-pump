...
namespace JsonPump.SquareEnix.ExFiles
{
    ...
    public class ExDRow
    {
        ...
        public Dictionary<ExHColumn, ExDData> Data = new();
        public int Key;
        public int Offset;
    ...
    public class ExDFile : ExFileBase
    {
        ...
        public ExDFile(SqFile sqFile, ExHFile exHFile)
        {
            ...
            var offsetEntriesSize = ToInt32(data, 0x8, true);
            var totalRowSize = ToInt32(data, 0xc, true);

            // Copy offset entries from raw data for easier manipulation.
            // Offset entries start from 0x20.
            var offsetEntries = new byte[offsetEntriesSize];
            ...
            for (var i = 0; i < offsetEntriesSize; i += 0x8)
            {
                ...
                var rawRowPosition = row.Offset - 0x20 - offsetEntriesSize;
                ...
                var fixedSizeData = new byte[exHFile.FixedSizeDataLength];
                ...
                var additionalData = new byte[row.Size - exHFile.FixedSizeDataLength];
                Array.Copy(rawRows, rawRowPosition + 0x6 + exHFile.FixedSizeDataLength, additionalData, 0,
                    additionalData.Length);

                // Let's go through each column of this row now.
                foreach (var column in exHFile.Columns)
                {
                    var parsedData = new ExDData();
                    ...
                    switch (column.Type)
                    {
                        ...
                        case 0x0:
                            ...
                            var valueStart = ToInt32(fixedSizeData, column.Offset, true);
                            ...
                            var field = new byte[valueEnd - valueStart];
        ...
        public void Iterate(Action<ExDRow> iterator)
        {
            for (var i = 0; i < Rows.Count; i++)
            {
                ...
                Utility.ReportInnerProgress(((double) i + 1) / Rows.Count);
            }
        }
        ...
        private byte[] DecodeTagByType(byte tagType, byte[] tag)
        {
            ...
            switch (tagType)
            {
                ...
                case 0x1a:
                    if (tag.Length != 1)
                        throw new Exception(
                            $"Error while decoding emphasis (0x1a) tag. Tag length is not 1: {PrintBytes(tag)}");
                    ...
                    else if (opening == 0) return new UTF8Encoding(false).GetBytes("</em>");

                    throw new Exception(
                        $"Error while decoding emphasis (0x1a) tag. Tag opening is not known: {opening}");
